...
- (id)initInScrollView:(UIScrollView *)scrollView activityIndicatorView:(UIView *)activity
{
    ...
    if (self) {
        self.backgroundColor = ODRefreshTintColor;
        ...
        _shapeLayer.fillColor = [ODRefreshShapeColor CGColor];
...
- (void)beginRefreshing
{
    if (!_refreshing) {
        ...
        // added by Andy@Tapatalk
        _activity.center = CGPointMake(floor(self.frame.size.width / 2), self.frame.size.height - kOpenedViewHeight/ 2);
        ...
        [UIView animateWithDuration:0.4 animations:^{
            ...
            // changed by Andy@Tapatalk
            [self.scrollView setContentOffset:CGPointMake(0, -kOpenedViewHeight) animated:NO];
        }];
